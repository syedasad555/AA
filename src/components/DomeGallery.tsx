      ...
      <main ref={mainRef} className="sphere-main">
        <div className="stage">
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => {
              const centerDistance = Math.sqrt(it.x * it.x + it.y * it.y);
              const maxDistance = Math.sqrt(37 * 37 + 5 * 5);
              const scaleFactor = 1 + (0.25 * (1 - Math.min(centerDistance / (maxDistance * 0.5), 1)));
              return (
              <div
                key={`${it.x},${it.y},${i}`}
                className="item"
